...
func SetUpRouter(app *fiber.App, model *model.Queries, rc redis.RedisCache, nsq nsq.Producer) {
	api := app.Group("/api")

	// Member
	memberService := member.NewService(model, rc)
	...
	// Attempt
	attemptService := attempt.NewService(model, rc, nsq)
